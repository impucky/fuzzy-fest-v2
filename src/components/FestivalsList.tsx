        ...
        .map((f) => {
          ...
          return (
            <li className="text-center" key={f.key}>
              <a
                className={`text-lg leading-snug font-black transition hover:text-[salmon] hover:underline md:text-xl ${pastFestival ? "text-neutral-400 line-through" : "text-white"}`}
                href={`/${year}/${f.key}`}
